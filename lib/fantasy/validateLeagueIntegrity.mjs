...
import { approvedRankingRefresh } from "./approvedRankingRefresh.ts";
...
const requiredArtifacts = [
  ...
  "data/workbookMarketReference.generated.json",
  "data/workbookMarketReferenceReceipt.generated.json",
];
...
if (await exists(requiredArtifacts[0])) {
  ...
  const candidateByName = new Map((warRoom.candidates ?? []).map((candidate) => [candidate.player?.fullName, candidate]));
  const incompleteProfileSignals = (warRoom.candidates ?? []).filter((candidate) => {
    const profile = candidate.signals?.profileCompleteness;
    return !profile || !Number.isFinite(profile.score) || profile.score < 0 || profile.score > 100 ||
      !Number.isFinite(profile.adjustmentScale) || profile.adjustmentScale < 0.35 || profile.adjustmentScale > 1;
  });
  if (incompleteProfileSignals.length > 0) {
    errors.push(`invalid profile-completeness signals ${incompleteProfileSignals.length}`);
  }
  const rookieIdentityConflicts = (warRoom.candidates ?? []).filter(
    (candidate) => candidate.advancedResearch?.lane?.startsWith("rookie-") && !candidate.player?.rookie,
  );
  if (rookieIdentityConflicts.length > 0) {
    errors.push(`rookie identity conflicts ${rookieIdentityConflicts.map((candidate) => candidate.player?.fullName).join(", ")}`);
  }
  const unsupportedCompetitionClaims = (warRoom.candidates ?? []).filter((candidate) => {
    const role = candidate.signals?.roleSecurity;
    if (!role || role.competitionEvidence) return false;
    const copy = [role.summary, ...(role.drivers ?? [])].join(" ");
    return role.competitionPressure !== null || /competition pressure/i.test(copy);
  });
  if (unsupportedCompetitionClaims.length > 0) {
    errors.push(`unsupported competition claims ${unsupportedCompetitionClaims.length}`);
  }
  const approvedRefreshMatches = approvedRankingRefresh.filter((entry) => {
    const candidate = candidateByName.get(entry.playerName);
    return candidate?.signals?.refresh?.lastUpdatedAt === "2026-09-01T23:59:00-04:00";
  });
  if (approvedRefreshMatches.length !== approvedRankingRefresh.length) {
    errors.push(`approved refresh coverage ${approvedRefreshMatches.length}/${approvedRankingRefresh.length}`);
  }
  const yahooXRankCoverage = (warRoom.candidates ?? []).filter(
    (candidate) => Number.isFinite(candidate.market?.yahooXRank),
  ).length;
  if (yahooXRankCoverage < 180) errors.push(`Yahoo XRank coverage ${yahooXRankCoverage}`);
}
...
if (await exists(requiredArtifacts[2]) && await exists(requiredArtifacts[3])) {
  const marketReference = JSON.parse(
    await readFile(new URL(requiredArtifacts[2], import.meta.url), "utf8"),
  );
  const receipt = JSON.parse(
    await readFile(new URL(requiredArtifacts[3], import.meta.url), "utf8"),
  );
  if (marketReference.sourceSheet !== "Source Data" || marketReference.workbookRows !== 200) {
    errors.push(`workbook market reference ${marketReference.sourceSheet ?? "missing"}/${marketReference.workbookRows ?? "missing"}`);
  }
  if (!receipt.accepted || receipt.ambiguousMatches?.length > 0 || receipt.invalidRanks?.length > 0) {
    errors.push("workbook market receipt is not accepted");
  }
  if ((receipt.coverage?.yahooXRank ?? 0) < 180 || (receipt.coverage?.matchedPlayers ?? 0) < 190) {
    errors.push(`workbook market coverage ${receipt.coverage?.matchedPlayers ?? 0}/${receipt.coverage?.yahooXRank ?? 0}`);
  }
}
