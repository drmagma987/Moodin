...
const MAKE_IT_RAIN_LEADERBOARD_COLLECTION = "makeItRainLeaderboard";
const MAX_LEADERBOARD_NAME_LENGTH = 11;
const LEADERBOARD_FETCH_LIMIT = 100;

export interface MakeItRainLeaderboardEntry {
  id: string;
  name: string;
  score: number;
  createdAtMs: number;
...
function sanitizeLeaderboardName(name: string) {
  ...
  return cleaned.slice(0, MAX_LEADERBOARD_NAME_LENGTH) || "BIG TIPPA";
}
...
export async function submitMakeItRainLeaderboardScore(name: string, score: number) {
  ...
  try {
    await addDoc(collection(db, MAKE_IT_RAIN_LEADERBOARD_COLLECTION), payload);
  } catch (error) {
...
export async function fetchMakeItRainLeaderboard() {
  await ensureAnonymousAuth();

  const leaderboardQuery = query(
    collection(db, MAKE_IT_RAIN_LEADERBOARD_COLLECTION),
    orderBy("score", "desc"),
  ...
  return snapshot.docs
    .map((doc) => {
      ...
      } satisfies MakeItRainLeaderboardEntry;
    })
    .sort((a, b) => {
