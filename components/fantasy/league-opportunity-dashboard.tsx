  ...
  return <div className="mt-4 space-y-4">
    ...
    {selectedPartner && selectedProfile ? <section className="rounded-[28px] border border-cyan-300/20 bg-[#0a1727]/95 p-4 sm:p-6">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-black uppercase text-rose-200">Clearest weaknesses</p><p className="mt-2 font-bold">{selectedPartner.whatTheyNeed.join(", ") || "No position grades below need threshold"}</p><div className="mt-3 flex flex-wrap gap-2">{GROUPS.map((group) => { const grade = selectedProfile.groups[group]; return <span key={group} className={cn("rounded-lg border px-2 py-1 text-xs font-black", grade.managerSatisfied ? "border-violet-300/25 bg-violet-300/10 text-violet-100" : gradeTone(grade.grade))}>{group} {grade.managerSatisfied ? "manager set" : grade.grade}</span>; })}</div></div>
